...
const Navbar = ({ filterHandler, searchHandler }) => {
    ...
    const menuItems =
        <>
            ...
            {!token &&
                <CustomLink onClick={() =>{ setModifiedNav(true);setShowBorder()}} className="btn-ghost hover:rounded lg:p-3 lg:m-5 font-bold" to='/sign-up'>Sign Up</CustomLink>

            }
            {!token &&
                <CustomLink onClick={() =>{ setModifiedNav(true);setShowBorder()}} className="btn-ghost hover:rounded lg:p-3 lg:m-5 font-bold" to='/sign-in'>Sign In</CustomLink>
            }
        </>
