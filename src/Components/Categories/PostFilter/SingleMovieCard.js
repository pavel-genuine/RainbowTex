    ...
    return (
        ...
        <Link to={`/movie-detail/${movie?._id}`}>
            <div className="border-b border-slate-800 pb-5 w-[90%] md:w-[100%] md:pl-0 pl-5">
                <article className="flex  my-3 flex-col lg:flex-row md:items-start md:space-x-6">
                    {
                movie?.thumbnail || movie?.videoCover ? <img className="flex-none w-60  rounded-md bg-slate-100" src={movie?.thumbnail ? movie?.thumbnail?.cdnUrl : movie?.videoCover?.cdnUrl} alt="" />
                    :
                    <img className="flex-none w-60  rounded-md bg-slate-100" src={'https://i.ibb.co/R6Y4CQ3/1-white-1.png'} alt="" />
